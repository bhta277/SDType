...
public class MaterializeSDTypes {
	...
	public void computeSDTypes() {
		...
		insert = "INSERT INTO dbpedia_untyped_instance (SELECT resource FROM stat_resource LEFT JOIN dbpedia_types USING (resource) LEFT JOIN dbpedia_disambiguations USING (resource) WHERE  dbpedia_types.resource IS NULL AND dbpedia_disambiguations.resource IS NULL);";
		try {
			stmt.execute(insert);
		} catch (SQLException e) {
...
//						System.out.println("Skipping " + resource + " after WordNet test");
						...
						String query2 = "SELECT type,SUM(tf*percentage*weight)/SUM(tf*weight) AS score FROM stat_resource_predicate_type WHERE resource='" + resourceEscaped + "' and type != 'http://www.w3.org/2002/07/owl#Thing' GROUP BY type HAVING score>="+ threshold + "ORDER BY score DESC LIMIT 1";
						Statement stmt2 = conn.createStatement();
						ResultSet RS2 = stmt2.executeQuery(query2);
						while(RS2.next()) {
							String type = RS2.getString(1);
//							FW.write("<" + resource + "> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <" + type + "> " + RS2.getDouble(2) + System.lineSeparator());
							FW.write("<" + resource + " -> " + type + "" + System.lineSeparator());
						}
						RS2.close();
					}
